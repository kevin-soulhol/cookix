      ...
      const scrollInterval = setInterval(() => {
        ...
        if (currentScrollY === lastScrollY) {
          ...
          if (unchangedScrollCount >= 3) {
            // 3 tentatives sans changement
            clearInterval(scrollInterval);
            console.log(
              "Fin de scroll: position inchangée après plusieurs tentatives"
            );
            resolve();
          }
        } else {
...
export async function getTestUserId(): Promise<number> {
  ...
  if (!user) {
    throw new Error(
      `Test user with email ${TEST_USER.email} not found in the database.`
    );
  }
  return user.id;
}
...
export async function clearShoppingListForTestUser() {
  try {
    ...
    if (shoppingList) {
      ...
      console.log(
        `🧹 Cleared ${count} shopping items for user ${TEST_USER.email}`
      );
    } else {
      console.log(
        `🧹 No shopping list found for user ${TEST_USER.email}, nothing to clear.`
      );
    }
  } catch (error) {
    console.error(
      `Error clearing shopping list for ${TEST_USER.email}:`,
      error
    );
...
export async function clearMenuItemsForTestUser() {
  try {
    ...
    if (menus.length > 0) {
      ...
      console.log(
        `🧹 No menus found for user ${TEST_USER.email}, no menu items to clear.`
      );
    }
  } catch (error) {
...
export async function addShoppingItemForTestUser(
  ...
  try {
    ...
    console.log(
      `➕ Added shopping item "${ingredientName}" for user ${TEST_USER.email}`
    );
    return newItem;
  } catch (error) {
    console.error(
      `Error adding shopping item "${ingredientName}" for ${TEST_USER.email}:`,
      error
    );
    throw error;
  }
